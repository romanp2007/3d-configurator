...
export function SaveLoadDialog({ mode, onClose, getThumbnail }: SaveLoadDialogProps) {
  const { listScenes, saveScene, loadScene, deleteScene, loading } = useSceneApi();
  ...
  async function handleSave() {
    ...
    if (result) {
      toast.success(`Сцена «${result.name}» сохранена`);
      await refreshList();
    } else {
                ...
                {scenes.map((scene) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-gray-400 text-xs mt-0.5">{formatDate(scene.updatedAt)}</div>
                    </div>
